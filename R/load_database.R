...
load_database <- function(
  ...
  if (is.null(url) || !nzchar(url)) {
    stop(
      "The selected .rds artifact does not have a usable download link.",
      call. = FALSE
    )
  }
...
order_version_rows <- function(publication_date, version_rank, id) {
  known_dates <- sort(
    unique(publication_date[!is.na(publication_date)]),
    decreasing = TRUE
  )
  ordered <- unlist(
    lapply(known_dates, function(date) {
